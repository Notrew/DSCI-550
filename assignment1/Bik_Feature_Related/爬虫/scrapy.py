...
def get_html_element(citation, primary_url: str) -> str:
...
def get_author_country(citation: str, url: str, driver):
    author_ref_local = {}
    driver.get(url)
    if part[0] in citation: 
...
def scrapy(x: pd.Series) -> str:
    DOI_url = x.DOI
    url = "https://doi.org/" + DOI_url
    html_url = get_html_element(x.Citation, url)
    author_result = get_author_country(x.Citation, html_url, driver)
    author_ref[DOI_url] = author_result


if __name__ == "__main__":
    df_ori = pd.read_excel("./Bik dataset - papers with endpoint reached.xlsx")
    df_ori.apply(scrapy, axis=1)
    print(author_ref)
